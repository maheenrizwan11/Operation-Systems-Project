...
void sha256_final(SHA256_CTX *ctx, BYTE hash[])
{
	...
	// Pad whatever data is left in the bufferr
	if (ctx->datalen < 56) {
		ctx->data[i++] = 0x80;
		while (i < 56)
			ctx->data[i++] = 0x00;
	...
	// Append to the padding the total message's length in bits, transform
...
void print_hash(BYTE buf[], int size) {
    ...
    for (int i = 0; i < size; i++) {
        byte_to_hexx(&hex_output[i * 2], buf[i]);  // byte to hex
    }
    hex_output[size * 2] = '\0';  // Null-terminate
    // printf("Hashed value: %s\n", hex_output);    // Print the final hex string
}

void sha256_compute_and_print(const BYTE *input) {
    BYTE buf[SHA256_BLOCK_SIZE];  // Buffer to hold the computed hash
    SHA256_CTX ctx;               // SHA-256 context

    sha256_init(&ctx);                            // initialize SHA-256 context
    sha256_update(&ctx, input, strlen((const char *)input));  // Update with input data
    sha256_final(&ctx, buf);                      // Finalize the hash computation

    print_hash(buf, SHA256_BLOCK_SIZE);    // Print the computed hash inhex
}
